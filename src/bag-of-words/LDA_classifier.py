...
def main():
    ...
    """
    # classifying orgs
    with open("dictionaries/LDA_categories_without_commons_1.json") as categories:
    ...
    with open("classifications/correct_classifications.json") as classifications:
        with open(
            "classifications/classifications_without_commons_LDA_1_all.json"
        ) as predictions:
...
def classify_org(category_data):
        ...
        for word in text:
            for category in category_data:
                if category_data[category].get(word) is not None:
                    category_scores[category] += category_data[category][word][
                        "freq"
                    ]  # score_word(word, category_data[category][word])
    ...
    # dumping data
    with open(
        "classifications/classifications_without_commons_LDA_1_all.json", "w"
...
def score_word_by_type(word, reference):
...
def score_word_by_relevance_multiplied(word, reference):
    tags = reference.get("tags")
    if tags is not None:
        for tag in reference["tags"]:
            if tag == "n":
