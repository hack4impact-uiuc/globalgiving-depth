...
def main():
    ...
    for project in training_data["projects"]:
        training_urls += [project["url"]] * len(project["tfidf_values"]) * len(project["themes"])
        training_themes += [theme["id"] for theme in project["themes"]] * len(project["tfidf_values"])
        training_values += [value for value in project["tfidf_values"]] * len(project["themes"])
        training_features += [feature for feature in project["features"]] * len(project["themes"])
